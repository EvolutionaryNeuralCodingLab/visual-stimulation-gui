classdef VS_linearlyMovingDotsWrapped < VStim
    properties (SetAccess=public)
        ...
        drifts = [0 0] %[N,2] matrix with drift in pixels/frame - the stronger the drift the more aligned are the dots. 
    ...
    properties (Constant)
        ...
        allDriftsTxt='[N,2] vector with each pair noting the [x,y] drift';
    ...
    methods
        ...
        function obj=run(obj)
            ...
            obj.allDrifts=nan(obj.nTotTrials,2);
            c=1;
            for i=1:nSpeeds
            ...
            if obj.randomize
                ...
                obj.allDrifts=obj.allDrifts(randomPermutation,:);
            end
            ...
            for i=1:obj.nTotTrials
                ...
                tmpDrift=obj.allDrifts(i,:);

                x=obj.xi{i};
                v=obj.vi{i};
                xAll=zeros([size(x),tmpNum]);
                xAll(:,1,1)=x(:,1)-obj.visualFieldRect(1);%remove offset for using modulu - this offset is added back before displaying dots
                xAll(:,2,1)=x(:,2)-obj.visualFieldRect(2);%remove offset for using modulu - this offset is added back before displaying dots
                for f=2:nFrames
                    xAll(:,1,f)=mod(xAll(:,1,f-1)+v(:,1)*obj.ifi(1)+tmpDrift(1),obj.visualFieldRect(3)-obj.visualFieldRect(1)); %calculate dynamics for x
                    xAll(:,2,f)=mod(xAll(:,2,f-1)+v(:,2)*obj.ifi(1)+tmpDrift(2),obj.visualFieldRect(4)-obj.visualFieldRect(2)); %calculate dynamics for y
                end
                xAll(:,:,:)=bsxfun(@plus,xAll(:,:,:),obj.visualFieldRect(1:2)); %add back offset of visual field
        ...
        %class constractor
        function obj=VS_linearlyMovingDotsWrapped(w,h)
