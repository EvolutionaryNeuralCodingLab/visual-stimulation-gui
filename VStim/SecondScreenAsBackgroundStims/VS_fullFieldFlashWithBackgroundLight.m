classdef VS_fullFieldFlashWithBackgroundLight < VStim
    properties (SetAccess=public)
        ...
        screen2LuminanceRGB=[0 0 255]; %in [R G B]
    ...
    properties (Constant)
        ...
        screen2LuminanceRGBTxt='[R G B] values for the background illumination of the second screen'
    ...
    methods
        function obj=run(obj)
            ...
            Screen('FillRect',obj.PTB_win(1),obj.luminosities(1),obj.rect(1,:));
            obj.applyBackgound(1);  %set background mask and finalize drawing (drawing finished)
            ...
            if ~obj.Back2Background  && obj.interTrialDelay~=0
               disp('Warning! Back2Background is false, while interTrialDelay>0! This means that the actual stim duration is not stimDuration!')
               pause;
            end

            if obj.useScreen2AsLightSource
                Screen('FillRect',obj.PTB_win(2),obj.screen2LuminanceRGB,obj.rect(2,:));
                Screen('DrawingFinished', obj.PTB_win(2));
                Screen('Flip',obj.PTB_win(2));
                obj.sendTTL(4,true);
            ...
            for i=1:obj.nTotTrials
                [obj.on_Flip(i),obj.on_Stim(i),obj.on_FlipEnd(i),obj.on_Miss(i)]=Screen('Flip',obj.PTB_win(1));
                obj.sendTTL(2,true);
                if obj.Back2Background %Display background between luminosities
                    % Update display
                    Screen('FillRect',obj.PTB_win(1),obj.visualFieldBackgroundLuminance,obj.rect(1,:));
                    obj.applyBackgound(1); %set background mask and finalize drawing (drawing finished)
                    [obj.off_Flip(i),obj.off_Stim(i),obj.off_FlipEnd(i),obj.off_Miss(i)]=Screen('Flip',obj.PTB_win(1),obj.on_Flip(i)+obj.actualStimDuration(1)-0.5*obj.ifi(1));
                    obj.sendTTL(2,false);
                ...
                % Update image buffer
                Screen('FillRect',obj.PTB_win(1),obj.luminosities(i+1),obj.rect(1,:));
                obj.applyBackgound(1); %set background mask and finalize drawing (drawing finished)
            ...
            Screen('FillRect',obj.PTB_win(2),[0 0 0],obj.rect(2,:));
            Screen('DrawingFinished', obj.PTB_win(2));
            Screen('Flip',obj.PTB_win(2));
            obj.sendTTL(4,false);
        ...
        function outStats=getLastStimStatistics(obj,hFigure)
            ...
            if nargin==2 & plotStats
                ...
                stimDurationShifts=(obj.off_Flip-obj.on_Flip)-obj.actualStimDuration(1);
        ...
        %class constractor
        function obj=VS_fullFieldFlashWithBackgroundLight(w,h)
